...
public class Or extends Builtin {
     ...
     */
    public Or(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public Or(Node builtinAtom, World world, Integer place) {
     ...
     * or not<br>
     * <br>
     * This function requires that the built-in atom has
     * exclusively one child element, which represents
     ...
     * <br>
     * Not functions return the execution result of another
     * built-in hosted in them. However, it reverses the 
     * execution success result from the other built-in.
     * All variable instantiates will be return exactly 
     * as they are from the execution of the other built-in.
     * 
     * @return Returns an execution result object.
     */
    public ExecutionResult execute() throws NotExecutableException {
        BuiltinManager builtinMan = new BuiltinManager(this.world, this.world.getBuiltinPath());
        ExecutionResult execResult = new ExecutionResult(); 
        ExecutionResult execResult1 = builtinMan.executeBuiltin(builtinAtom.childNode(0), this.place);
        ExecutionResult execResult2 = builtinMan.executeBuiltin(builtinAtom.childNode(1), this.place);
        execResult.setSuccess(execResult1.isSuccess() || execResult2.isSuccess());
        return execResult;
    }
