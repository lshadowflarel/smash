...
/**
 * GetInMemoryObject Get the persisted object by "key"
 * 
 * @author Paramai Supadulchai
 */
public class DirectInput extends Builtin {
	 ...
	 */
	public DirectInput(Node builtinAtom, World world, Integer place,
			PrintStream out) {
	...
	public Boolean isMatched() {
		return builtinAtom.hasAttributeByLocalName(ATTR_NAME)
	...
	public Boolean isExecutable() {
		return !builtinAtom.attributeByLocalName(ATTR_NAME).isVariable()
	...
	public ExecutionResult execute() throws NotExecutableException {
		...
		String answer = "";
		if (this.getWorld().getExecutor().getDirectInputs().getDirectInput().containsKey(builtinAtom
				.attributeByLocalName(ATTR_NAME).getNodeValue()))
			answer = this.getWorld().getExecutor().getDirectInputs().getDirectInput().get(builtinAtom
					.attributeByLocalName(ATTR_NAME).getNodeValue());

		spec.addSpecialization(builtinAtom.attributeByLocalName(ATTR_RESULT)
				.getNodeValue(), answer);
