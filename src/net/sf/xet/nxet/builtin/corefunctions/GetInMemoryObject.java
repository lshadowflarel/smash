 ...
 */
public class GetInMemoryObject extends Builtin {
	 ...
	 */
	public GetInMemoryObject(Node builtinAtom, World world, Integer place,
			PrintStream out) {
	...
	public ExecutionResult execute() throws NotExecutableException {
		...
		String val = this.getWorld().getExecutor().getInMemoryObjects().getInMemoryObject()
				.get(builtinAtom.attributeByLocalName(ATTR_KEY).getNodeValue());
		
		spec.addSpecialization(builtinAtom.attributeByLocalName(ATTR_RESULT)
				.getNodeValue(), val!=null?val:"");
