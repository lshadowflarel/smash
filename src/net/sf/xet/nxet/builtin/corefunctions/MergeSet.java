 ...
 */
public class MergeSet extends Builtin {

    public static final String ELEMENT_FIRST_SET = "FirstSet";
    public static final String ELEMENT_SECOND_SET = "SecondSet";
     ...
     */
    public MergeSet(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public MergeSet(Node builtinAtom, World world, Integer place) {
    ...
    public Boolean isMatched() {
        return new Boolean((builtinAtom.numberOfChildNodes() == 3) &&
                		   (builtinAtom.childNode(0).getNodeName().equals(ELEMENT_FIRST_SET)) &&
                		   (builtinAtom.childNode(0).getUri().equals(builtinAtom.getUri())) &&
                		   (builtinAtom.childNode(1).getNodeName().equals(ELEMENT_SECOND_SET)) &&
                		   (builtinAtom.childNode(1).getUri().equals(builtinAtom.getUri())) &&
                		   (builtinAtom.childNode(2).getNodeName().equals(ELEMENT_RESULT_SET))&&
    ...
    public ExecutionResult execute() throws NotExecutableException {
    	...
    	if(builtinAtom.childNode(0).getChildNodes().size()==0 || builtinAtom.childNode(1).getChildNodes().size()==0) {
    		...
    		if(builtinAtom.childNode(0).getChildNodes().size()>0) resultSet = builtinAtom.childNode(0).cloneNode();
    		else if(builtinAtom.childNode(1).getChildNodes().size()>0) resultSet = builtinAtom.childNode(1).cloneNode();
        ...
        Node firstSet = builtinAtom.childNode(0); 
        Node secondSet = builtinAtom.childNode(1); 
        Node resultSet = builtinAtom.childNode(2).childNode(0);
        
//        System.out.println("FIRST SET");
//        System.out.println(firstSet.printXML());
//        
//        System.out.println("\nSECOND SET");
//        System.out.println(secondSet.printXML());
		...
		Node root = firstSet.childNode(0).cloneNode();
		root.removeAllNodes();
		
        // Add all element in the first set into result set
        for (int j = 0; j < firstSet.numberOfChildNodes(); j++) {
        	for(int k=0; k<firstSet.childNode(j).numberOfChildNodes(); k++) {
        		root.addChildNode(firstSet.childNode(j).childNode(k).cloneNode());
        	}
        }
        
        // Add all element in the second set into result set
        for (int i = 0; i < secondSet.numberOfChildNodes(); i++) {
        	found = false;
            for (int j = 0; j < firstSet.numberOfChildNodes(); j++) {
            	MatchResult matchResult = matcher.match(firstSet.childNode(j), secondSet.childNode(i), Matcher.MODE_SEQ_D);
            ...
            if(!found) {
            	for(int k=0; k<secondSet.childNode(i).numberOfChildNodes(); k++) {
            		root.addChildNode(secondSet.childNode(i).childNode(k).cloneNode());
            	}
            }
        }
//        System.out.println("\nRESULT SET");
