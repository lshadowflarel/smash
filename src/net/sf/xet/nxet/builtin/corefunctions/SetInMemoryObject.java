...
/**
 * SetInMemoryObject Set the "value" of the persisted object by "key"
 * 
 * @author Paramai Supadulchai
 */
public class SetInMemoryObject extends Builtin {
	 ...
	 */
	public SetInMemoryObject(Node builtinAtom, World world, Integer place,
			PrintStream out) {
	 ...
	 * <br>
	 * This function requires three specific attributes
	 * 
	...
	public Boolean isMatched() {
		return builtinAtom.hasAttributeByLocalName(ATTR_KEY)
				&& builtinAtom.hasAttributeByLocalName(ATTR_VAL);
	}
	 ...
	 * uninstantiated while all the other nodes are instantiated
	 * 
	...
	public Boolean isExecutable() {
		return !builtinAtom.attributeByLocalName(ATTR_KEY).isVariable()
				&& !builtinAtom.attributeByLocalName(ATTR_VAL).isVariable();
	}
	...
	public ExecutionResult execute() throws NotExecutableException {
		...
		String key = builtinAtom.attributeByLocalName(ATTR_KEY).getNodeValue();
		String value = builtinAtom.attributeByLocalName(ATTR_VAL)
				.getNodeValue();

		this.getWorld().getExecutor().getInMemoryObjects().getInMemoryObject().put(key, value);
